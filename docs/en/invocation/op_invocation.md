...
Operators can be invoked through multiple methods. This chapter uses the `AddExample` operator invocation as an example to detail the operator invocation and execution process.

> Note: The aclnn interface invocation method is recommended first. If this method is not supported, use the graph construction method to invoke operators.

- aclnn Invocation **(Recommended)**: Invoke operators using the operator aclnnXxx interface method (a set of C-language-based APIs, no IR definition required).
- Graph Mode Invocation: Invoke operators using the IR (Intermediate Representation) graph construction method.
...
The sample code for invoking the `AddExample` operator using the aclnn interface is as follows (for detailed code, refer to [test_aclnn_add_example.cpp](../../../examples/add_example/examples/test_aclnn_add_example.cpp)). **This is for reference only**. Other operator interface invocation processes are similar; replace with the actual aclnn interface. Before invocation, set environment variables according to the environment installation prompt information.

Note: To invoke other operators in this project, access the test_aclnn_${op_name}.cpp in the corresponding operator's `examples` directory, where ${op_name} represents the operator name.

```Cpp
int main()
{
    // 1. Invoke acl for device/stream initialization
    int32_t deviceId = 0;
    aclrtStream stream;
    auto ret = Init(deviceId, &stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("Init acl failed. ERROR: %d\n", ret); return ret);

    // 2. Construct input and output, customize according to the API interface
    ...
    // 3. Invoke the CANN operator library API, modify to the specific API name
    uint64_t workspaceSize = 0;
    aclOpExecutor* executor;

    // 4. Invoke the aclnnAddExample first-stage interface
    ...
    // 5. Apply for device memory based on the workspaceSize calculated by the first-stage interface
    ...
    // 6. Invoke the aclnnAddExample second-stage interface
    ...
    // 7. (Fixed pattern) Synchronously wait for task execution to complete
    ret = aclrtSynchronizeStream(stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclrtSynchronizeStream failed. ERROR: %d\n", ret); return ret);

    // 8. Get the output value, copy the result from device-side memory to host-side, modify according to the specific API interface definition
    PrintOutResult(outShape, &outDeviceAddr);

    // 9. Release aclTensor, modify according to the specific API interface definition
...
> Note: For operators already implemented in the project (non-custom operators), you can directly run operators through the [build.sh](../../../build.sh) in the root directory. For operations, refer to [Local Verification](./quick_op_invocation.md#local-verification).

1. Prerequisites.
   Complete the compilation and deployment of the target operator by referring to [Source Code Compilation](./quick_op_invocation.md#source-code-compilation) in this project.

2. Create CMakeLists file.

   Create a CMakeLists file in the same directory as test_aclnn_${op_name}.cpp. Using the `AddExample` operator as an example, the sample is as follows. Modify according to actual conditions.
   ...
   # Set compilation output directory to the bin folder under the current directory
   ...
   # Split path list and find the path containing /vendors/ (only needed for custom operators)  
   string(REPLACE ":" ";" LD_LIB_LIST "${LD_LIB_PATH}")
   set(TARGET_PATH "")
   foreach(path ${LD_LIB_LIST})
       # Match paths containing /vendors/ (position-independent)
   ...
   # Add executable file (replace with actual operator executable file), specify the *.cpp file for operator invocation
   add_executable(test_aclnn_add_example              
                  test_aclnn_add_example.cpp)

   # ASCEND_PATH (CANN software package directory, modify according to actual path)
   ...
   target_link_libraries(test_aclnn_add_example PRIVATE             # Replace with actual operator executable file
       ...
       # ${ASCEND_PATH}/lib64/libopapi_transformer.so    # Only needed for built-in operators
   )
...
3. Create run.sh file.
    ...
    Create a run.sh file in the same directory as test_aclnn_${op_name}.cpp. Using the `AddExample` operator as an example, the sample is as follows. Modify according to actual conditions.
...
4. Run run.sh file.
    Execute the following command in the path where the run.sh file is located:
...
The sample code for invoking the `AddExample` operator using the graph method is as follows (for detailed code, refer to [test_geir_add_example.cpp](../../../examples/add_example/examples/test_aclnn_add_example.cpp)). **This is for reference only**. Other operator invocation processes are similar; replace with the actual operator prototype.

To invoke other operators in this project, access the test_geir_${op_name}.cpp in the corresponding operator's `examples` directory, where ${op_name} represents the operator name.
...
int main() {
    ...
    // 6. Macro expansion method for variable assignment
    ...
    // 7. Set graph object input operators and output operators
...
2. Create CMakeLists file.
   ...
   Create a CMakeLists file in the same directory as test_geir_${op_name}.cpp. Using the `AddExample` operator as an example, the sample is as follows. Modify according to actual conditions.
   ...
   # Add executable file (replace with actual operator executable file)
...
3. Create run.sh script.
   ...
   Create a run.sh file in the same directory as test_geir_${op_name}.cpp. Using the `AddExample` operator as an example, the sample is as follows. Modify according to actual conditions.
