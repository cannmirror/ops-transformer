# TensorScalar Mutual Derivation Relationship

## Derivation Rules

When an API (such as aclnnAdds, aclnnMuls) has **inconsistent input Tensor data type** and **input Scalar data type**, the API internally derives a data type and converts the input data to that data type for computation.
...
>- The first row of the table represents the input Tensor data type to be derived, the first column represents the input Scalar data type to be derived, and the corresponding position in the table represents the derived data type.
...
- If the input Tensor data type is float16 and the input Scalar data type is float32, the API internally converts the input Scalar float32 data type to float16 data type for computation.
- If the input Tensor data type is bool and the input Scalar data type is float32, the API internally converts the input Tensor bool data type to float32 data type for computation.
