...
Currently, most operator API inputs of aclTensor support "**non-contiguous Tensor**", meaning a Tensor can be represented by (shape, strides, offset).

Note: Creating aclTensor can be implemented through [Operator Library Interface](https://www.hiascend.com/document/detail/en/CANNCommunityEdition/latest/API/aolapi/operatorlist_00001.html) in "Common Interface > aclCreateTensor".

## Sample 1

For example, a Tensor with shape=(6, 5), strides=(10, 1), offset=22 has the following memory layout:
...
The Tensor layout corresponds to the highlighted positions above. This complete Tensor is non-contiguous in memory layout. Strides describe the interval between two adjacent elements on a Tensor dimension. If the stride on dimension 1 is 1, that dimension is contiguous; if the stride on dimension 0 is 10, adjacent elements are separated by 10 elements, meaning non-contiguous. Offset represents the offset of the first element of this Tensor relative to addr.

## Sample 2

For example, a Tensor with shape=(4, 3), strides=(20, 2), offset=22 has the following memory layout:
...
The Tensor layout corresponds to the highlighted positions above. This complete Tensor is non-contiguous in memory layout. Strides describe the interval between two adjacent elements on a Tensor dimension. If the stride on dimension 1 is 2, elements on that dimension are separated by 1 element; if the stride on dimension 0 is 20, adjacent elements are separated by 20 elements, meaning non-contiguous. Offset represents the offset of the first element of this Tensor relative to addr.
