...
def parse_block_stages(data: bytes, offset: int, num_blocks: int, stage_num: int):
    ...
    block_stage_size = 8 + stage_num * 8  # uint64 currentStage + uint64 stageCycle[stageNum]
    results = []
...
def parse_dump_params(data: bytes, offset: int, dump_count: int):
    ...
    for i in range(dump_count):
        ...
        if dp_offset + DUMP_PARAMS_SIZE > len(data):
            break
        fields = struct.unpack(DUMP_PARAMS_FORMAT, data[dp_offset:dp_offset + DUMP_PARAMS_SIZE])
        results.append({
            'index': i,
            'dumpAddr': fields[0],
            'blockCount': fields[1],
...
def parse_dump_entry_data(data: bytes, offset: int, dp: dict):
    ...
    if offset + total_size > len(data):
        logging.warning("dump entry %d 数据不完整: 需要 %d 字节, 剩余 %d 字节",
                        dp['index'], total_size, len(data) - offset)
...
def analyze_block_stages(block_stages: list):
    ...
    for stage_id, count in sorted(stage_counter.items()):
        ...
        logging.info("  stage=%d (%s): %d 核 (%.1f%%)",
                     stage_id, name, count, count * 100.0 / len(block_stages))
    ...
    for bs in block_stages:
        ...
        for _, c in enumerate(bs['stageCycle']):
            if c != 0:
                last_cycle = c
        logging.info("  core %2d: stage=%d (%s), lastCycle=0x%x",
                     bs['coreId'], bs['currentStage'], name, last_cycle)

    # 定位卡死阶段
    stuck_name = get_stage_name(majority_stage)
    logging.warning(">>> 疑似卡死阶段: %s (%d/%d 核, %.1f%%)",
                    stuck_name, majority_count, len(block_stages),
                    majority_count * 100.0 / len(block_stages))

    # 找出不在多数阶段的核
    minority_cores = [bs['coreId'] for bs in block_stages
                      if bs['currentStage'] != majority_stage]
    if minority_cores:
...
def analyze_dump_entries(dump_params: list, dump_data_offset: int, data: bytes):
    ...
    for dp in dump_params:
        ...
        logging.info("[%d] %s: addr=0x%x, blockCount=%d, blockLen=%d, srcStride=%d",
                     dp['index'], name, dp['dumpAddr'], dp['blockCount'],
                     dp['blockLen'], dp['srcStride'])
...
def analyze_per_rank_status(raw: np.ndarray, dp: dict):
    ...
    logging.info("  per_rank_status: %d 个 rank, 每 rank %dB", dp['blockCount'], dp['blockLen'])
    for i in range(min(dp['blockCount'], 32)):
        offset = i * dp['blockLen']
...
def analyze_self_status(raw: np.ndarray, dp: dict):
    ...
    if statuses:
        ...
        logging.info("    active(1.0f): %d, inactive(0): %d, other: %d",
                     active, inactive, len(statuses) - active - inactive)
        logging.info("    前 %d 个值: %s", len(statuses),
                     [hex(v & 0xFFFFFFFF) for v in statuses])
...
def main():
    ...
    if tiling_data_size > 0 and tiling_end <= len(data):
        ...
        logging.info("大小: %d 字节 (原始二进制, 需按 TilingDataT 结构体解析)\n", tiling_data_size)
    else:
        logging.warning("Tiling 数据不可用\n")

    # 3. 解析 BlockStage[]
    block_stage_offset = tiling_offset + tiling_data_size
    block_stages = parse_block_stages(data, block_stage_offset, num_blocks, hdr['stageNum'])
