...
def cal_relative_diff_mix(real_data, expect_data, diff_thd):
    if (
        "nan" in str(expect_data)
        or "inf" in str(expect_data)
...
def display_output(
    real_data, expect_data, start, end, diff_thd, expect_fp32_data=None, if_mix=False
):
    def display_inner(idx):
        j = idx + start
        if if_mix:
            diff_rate = cal_relative_diff_mix(expect_data[j], real_data[j], diff_thd)
        else:
...
def compare(expect_data, real_data):
    ...
    diff_abs = np.abs(
        np.subtract(real_data.astype(np.float32), expect_data.astype(np.float32))
    )
    ...
    if result == "Failed":
        display_error_output(real_data, expect_data, err_idx, err_diff[0:max_error_idx])
    return result == "Pass"
