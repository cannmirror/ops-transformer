...
__aicore__ inline void CopyQueryGmToL1(FaL1Tensor<Q_T> &dstTensor, FaGmTensor<Q_T, GM_FORMAT> &srcTensor,
                                       GmCoord &gmCoord)
{
    ...
    uint64_t queryGmbaseOffset = GetOffset(info, gmCoord.bIdx, gmCoord.n2Idx, 0, s1IdxStart, gmCoord.dIdx);
    ...
    CopySingleMatrixNDToNZ(dstTensor.tensor, srcTensor.gmTensor[offset], headSize, gmCoord.dDealSize, info.strideG,
                           dstTensor.rowCount);
    ...
    if (s1IdxEnd - s1IdxStart >= 1) {
        ...
        if (s1IdxEnd - s1IdxStart > 1) {
            CopyMultiMatrixNDToNZ(dstTensor.tensor[l1Offset], srcTensor.gmTensor[gmOffset], s1IdxEnd - s1IdxStart - 1,
                                  info.strideS1, info.dimG * blockElementCnt, info.dimG, gmCoord.dDealSize,
        ...
        // 处理尾块
        if (gIdxEnd > 0) {
            CopySingleMatrixNDToNZ(dstTensor.tensor[l1Offset], srcTensor.gmTensor[gmOffset], gIdxEnd, gmCoord.dDealSize,
                                   info.strideG, dstTensor.rowCount);
        }
    }
}
...
__aicore__ inline void CopyKvGmToL1(FaL1Tensor<KV_T> &dstTensor, FaGmTensor<KV_T, GM_FORMAT> &srcTensor,
                                    GmKvCoord &gmCoord)
{
    // B*N2*GS1*D
    auto &info = srcTensor.offsetInfo;
                                         ...
                                         GmCoord &gmCoord)
{
    ...
    uint32_t srcStride = (srcTensor.colCount - gmCoord.dDealSize) / (fa_kernel::hardware::BYTE_BLOCK / sizeof(OUT_T));
