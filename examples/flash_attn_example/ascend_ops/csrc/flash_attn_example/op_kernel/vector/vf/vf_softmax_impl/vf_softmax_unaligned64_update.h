...
__simd_vf__ void ProcessVec1UpdateImpl64VF(__ubuf__ T2 *expUb, __ubuf__ T *maxUb, __ubuf__ T *srcUb,
                                           ...
                                           __ubuf__ uint32_t *maskUb, const uint32_t nPadding,
                                           const uint32_t blockStride, const uint32_t repeatStride, const float dScale,
                                           uint32_t pltOriginalN, uint32_t pltSrcN, uint32_t pltSrcN16,
...
ProcessVec1UpdateImpl64(const LocalTensor<T2> &dstTensor, const LocalTensor<uint8_t> &indexesTensor,
                        ...
                        const uint16_t m, const uint32_t originN, const T scale, const T minValue)
{
    ...
    ProcessVec1UpdateImpl64VF<T, T2, s1BaseSize, s2BaseSize, hasAttn>(
        expUb, maxUb, srcUb, expMaxUb, inMaxUb, tmpExpSumUb, tmpMaxUb, tmpMaxUb2, indexesUb, maskUb, nPadding,
        blockStride, repeatStride, scale, pltOriginalN, pltSrcN, pltSrcN16, m, scale, minValue);
}
#endif
} // namespace FaVectorApi
