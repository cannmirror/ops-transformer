...
class MoeDistributeBuffer:
    def __init__(self, group, comm_alg: int = 0):
        ...
        self.context, self.ccl_buffer_size = torch.ops.ascend_ops.updateContext(
            x_context, self.group_name, self.world_size
        )

    @staticmethod
    def get_ccl_buffer_size(
        world_size: int,
    ...
    def npu_moe_distribute_dispatch_v2(
        self,
        x,
        expert_ids,
        moe_expert_num,
    ...
    def npu_moe_distribute_combine_v2(
        self,
        expand_x,
        expert_ids,
