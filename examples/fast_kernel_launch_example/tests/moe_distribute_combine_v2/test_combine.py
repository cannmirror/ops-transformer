...
def run_npu_process(rank):
    ...
    if cfg.is_dispatch_scales:
        shape = (
            (1 + cfg.num_experts, cfg.hidden_size)
            if cfg.shared_expert_rank_num
            else (cfg.num_experts, cfg.hidden_size)
        )
    ...
    expand_x, _, assist_info, _, ep_recv_counts, _ = dispatch_fn(
        ...
        const_expert_num=cfg.const_expert_num,
    )
    ...
    _ = combine_fn(
        ...
        global_bs=cfg.global_batch_size,
        ori_x=x,
        const_expert_alpha_1=const_alpha_1,
        const_expert_alpha_2=const_alpha_2,
        const_expert_v=const_v,
        zero_expert_num=cfg.zero_expert_num,
        copy_expert_num=cfg.copy_expert_num,
        const_expert_num=cfg.const_expert_num,
    )
...
def validate_config():
    ...
    if cfg.shared_expert_rank_num > cfg.ep_world_size:
        print("shared_expert_rank_num > ep_world_size")
        return False
    if (
        cfg.shared_expert_rank_num > 0
        and cfg.ep_world_size % cfg.shared_expert_rank_num != 0
    ):
        print("ep_world_size must be multiple of shared_expert_rank_num")
        return False
    if cfg.num_experts % cfg.moe_rank_num != 0:
        print("num_experts must be multiple of moe_rank_num")
        return False
    return True
