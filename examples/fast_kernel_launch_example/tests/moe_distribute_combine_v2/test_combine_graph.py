...
class Config:
    ...
    def __init__(self):
        ...
        self.shared_expert_rank_num = (
            self.shared_expert_num * self.rank_num_per_shared_expert
        )
...
buff_size = MoeDistributeBuffer.get_ccl_buffer_size(
    cfg.world_size,
    cfg.batch_size,
    cfg.hidden_size,
    cfg.moe_expert_num,
    cfg.topk,
    cfg.shared_expert_num,
...
def build_model_kwargs(
    x, expert_ids, x_active_mask, group_ep, ep_rank_id, expert_scales, ori_x
):
    """Build keyword arguments for model forward."""
    mask = (
        x_active_mask.to(torch.bool).npu()
        if (cfg.is_mask or cfg.is_2d_mask) and x_active_mask is not None
        else None
    )
    return {
...
def run_cascade_npu(queue, rank, x, expert_ids, x_active_mask, expert_scales, ori_x):
    ...
    (
        ep_hcomm,
        ep_group,
    ) = init_hccl_comm(rank, ep_ranks)

    kwargs = build_model_kwargs(
        x, expert_ids, x_active_mask, ep_hcomm, rank, expert_scales, ori_x
    )
...
def parse_rank_input(rank: int, server_kwargs: Dict) -> Dict:
    ...
    return {
        "queue": server_kwargs["result_queue"],
        "rank": rank,
...
def gen_x() -> torch.Tensor:
    ...
    return (
        torch.arange(1, cfg.global_batch_size + 1, dtype=cfg.input_dtype)
        .view(-1, 1)
        .repeat(1, cfg.hidden_size)
    )
...
def gen_expert_ids() -> torch.Tensor:
    ...
    def gen_balanced(round_idx):
        return (
            torch.arange(round_idx, round_idx + cfg.moe_expert_num, dtype=torch.int32)
            % cfg.moe_expert_num
        )
    ...
    if cfg.is_topk_balance:
        ...
        ids = [
            gen_random(cfg.topk, 0, cfg.moe_expert_num)
            for _ in range(cfg.global_batch_size)
        ]
    result = torch.cat(ids)[: cfg.global_batch_size * cfg.topk].view(
        [cfg.global_batch_size, cfg.topk]
    )
    return result
...
def gen_x_active_mask() -> torch.Tensor:
    ...
    def gen_card_mask():
        ...
        return torch.cat(
            (torch.ones(real_num), torch.zeros(cfg.batch_size - real_num))
        ).to(torch.bool)
...
def gen_expert_scales() -> torch.Tensor:
    ...
    return torch.empty([cfg.global_batch_size, cfg.topk], dtype=torch.float32).uniform_(
        -1, 1
    )
