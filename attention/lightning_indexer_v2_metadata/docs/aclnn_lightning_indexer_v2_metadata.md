...
- 算子功能：该算子为AICPU算子，是aclnnLightningIndexerV2算子的前置算子。根据aclnnLightningIndexerV2算子的输入信息，计算并输出负载均衡结果。输出结果可以作为aclnnLightningIndexerV2算子的输入，减少aclnnLightningIndexerV2算子的执行耗时。

  **该算子不建议单独使用，建议与aclnnLightningIndexerV2算子配合使用，形成完整的工作流。**
    1. 接受aclnnLightningIndexerV2算子输入数据shape信息，包含batchSize，qSeqlen，kSeqlen, mask。通过对输入分块并模拟计算耗时，均匀分配分块到可用核上，以降低aclnnLightningIndexerV2算子的整体计算耗时，并提高硬件利用率。
...
## 约束说明
  - aclnnLightingIndexerV2Metadata默认为确定性实现，暂不支持非确定性实现，确定性计算配置也不会生效。
...
| 调用方式  | 样例代码                                                                | 说明                                                                                          |
| ----------- | ------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| aclnn接口 | [test_aclnn_lightning_indexer_v2_metadata.cpp](./examples/test_aclnn_lightning_indexer_v2_metadata.cpp) | 通过[`aclnnLightningIndexerV2Metadata`](./docs/aclnnLightningIndexerV2Metadata.md)接口方式调用 |
