...
namespace fa_split_core_v2 {     
    ...
    {
        if (baseInfo.cuSeqS1Size.empty()) {
            return baseInfo.s1Size;
        }

        if (baseInfo.cuLenQDims == 1U) {
            return static_cast<uint32_t>(baseInfo.cuSeqS1Size[0]);
        }

        if (!baseInfo.isAccumSeqS1) {
            return static_cast<uint32_t>(baseInfo.cuSeqS1Size[bIdx]);
        }

        return (bIdx == 0) ? static_cast<uint32_t>(baseInfo.cuSeqS1Size[bIdx]) :
               static_cast<uint32_t>(baseInfo.cuSeqS1Size[bIdx] - baseInfo.cuSeqS1Size[bIdx - 1U]);
    }

    uint32_t GetS2SeqSize(uint32_t bIdx, const BaseInfo &baseInfo)
    {
        uint32_t prefix = static_cast<uint32_t>(baseInfo.cuSeqPrefixSize);
        if (baseInfo.cuSeqS2Size.empty()) {
            return prefix + baseInfo.s2Size;
        }

        if (baseInfo.cuLenKvDims == 1U) {
            return prefix + static_cast<uint32_t>(baseInfo.cuSeqS2Size[0]);
        }

        if (!baseInfo.isAccumSeqS2) {
            return prefix + static_cast<uint32_t>(baseInfo.cuSeqS2Size[bIdx]);
        }

        return (bIdx == 0) ? prefix + static_cast<uint32_t>(baseInfo.cuSeqS2Size[bIdx]) :
               prefix + static_cast<uint32_t>(baseInfo.cuSeqS2Size[bIdx] - baseInfo.cuSeqS2Size[bIdx - 1U]);
    }

    int64_t CalcPreTokenLeftUp(uint32_t s1Size, uint32_t s2Size, const BaseInfo &baseInfo)
    {
        auto mode = static_cast<SparseMode>(baseInfo.maskMode);
        if (mode == SparseMode::BAND) {
            return static_cast<int64_t>(s1Size) - static_cast<int64_t>(s2Size) + baseInfo.winLeft;
        }
        return baseInfo.winLeft;
    }
    ...
    void CalcSplitPlan(uint32_t coreNum, int64_t costLimit, const SplitContext &splitContext, FAMetaData &result)
    {
        ...
        for (uint32_t i = 0; i < coreNum; ++i) {
            ...
            result.headFdDataIdx[assignContext.curCoreIdx] = assignContext.preFdDataNUM + assignContext.curKvSplitPart - 1U;
    ...
    void SplitFD(FAMetaData &result)
    {
        ...
        for (uint32_t i = 0; i < result.fdRes.fdNum; i++) {
            uint32_t curFDVectorNum = fdRes.fdS2SplitNum[i] * fdRes.mSize[i] / averageLoad; // 计算当前归约任务所用核数，向下取整，避免使用核数超出总核数
            uint32_t curAvgMSize = (fdRes.mSize[i] + curFDVectorNum - 1U) / curFDVectorNum; // 计算当前归约任务每个核的行数，向上取整，避免行数为0
            curFDVectorNum = (fdRes.mSize[i] + curAvgMSize - 1U) / curAvgMSize; // 重新计算正确的核数，避免因为向上取整导致有核为空
    ...
    void SplitCore(uint32_t coreNum,const BaseInfo &baseInfo, const SplitParam &param, FAMetaData &result)
    {
        ...
        printf("s2Size: %u\n", baseInfo.s2Size);
        printf("isS1G: %u\n", baseInfo.isS1G);
        printf("isAccumSeqS1: %u\n", baseInfo.isAccumSeqS1);
        printf("isAccumSeqS2: %u\n", baseInfo.isAccumSeqS2);
        printf("cuLenQDims: %u\n", baseInfo.cuLenQDims);
        printf("cuLenKvDims: %u\n", baseInfo.cuLenKvDims);
        printf("attenMaskFlag: %u\n", baseInfo.attenMaskFlag);
        printf("sparseMode: %u\n", baseInfo.maskMode);
        printf("preToken: %lld\n", baseInfo.winLeft);
        printf("nextToken: %lld\n", baseInfo.winRight);
        printf("cuSeqPrefixSize: %lld\n", baseInfo.cuSeqPrefixSize);
        printf("result.usedCoreNum: %d \n", result.usedCoreNum);

        // 打印 cuSeqS1Size
        if (baseInfo.cuSeqS1Size.empty()) {
            printf("cuSeqS1Size: nullptr\n");
        } else {
            printf("cuSeqS1Size: [");
            for (size_t i = 0; i < baseInfo.cuSeqS1Size.size(); ++i) {
                printf("%lld", baseInfo.cuSeqS1Size[i]);
                if (i < baseInfo.cuSeqS1Size.size() - 1) {
                    printf(", ");
                }
            }
            printf("]\n");
    ...
    // 打印 cuSeqS2Size
        if (baseInfo.cuSeqS2Size.empty()) {
            printf("cuSeqS2Size: nullptr\n");
        } else {
            printf("cuSeqS2Size: [");
            for (size_t i = 0; i < baseInfo.cuSeqS2Size.size(); ++i) {
                printf("%lld", baseInfo.cuSeqS2Size[i]);
                if (i < baseInfo.cuSeqS2Size.size() - 1) {
                    printf(", ");
                }
            }
            printf("]\n");
        ...
        if (result.usedCoreNum - tmpResult.usedCoreNum < gapCoreNum){
            if (tmpResult.maxCost - result.maxCost < gapBlockNum * CalcCost(std::min(maxFdSize,param.mBaseSize),param.s2BaseSize)){
                CopyTmpResult(tmpResult,result);
            }
        }
        else {
            if (tmpResult.maxCost - result.maxCost < (gapBlockNum + 1) * CalcCost(std::min(maxFdSize,param.mBaseSize),param.s2BaseSize)){
                CopyTmpResult(tmpResult,result);
            }
        }
        result.usedCoreNum = std::max(result.usedCoreNum, 1U);  // 至少使用1个core

        printf("========== start print split core res ==========\n");
        printf("result.usedCoreNum: %d \n", result.usedCoreNum);
        printf("result.maxCost: %lld \n", result.maxCost);
        printf("result.fdRes.fdNum: %d \n", result.fdRes.fdNum);
        printf("result.fdRes.fdUsedVecNum: %d \n", result.fdRes.fdUsedVecNum);
        for (uint32_t i = 0; i < result.usedCoreNum; i ++) {
            printf("outerSplitParams.bN2End[%d]: %d \n", i, result.bN2End[i]);
            printf("outerSplitParams.mEnd[%d]: %d \n", i, result.mEnd[i]);
            printf("outerSplitParams.s2End[%d]: %d \n", i, result.s2End[i]);
            printf("fDParams.headFdDataIdx[%d]: %d \n", i, result.headFdDataIdx[i]);
        }

        for (uint32_t i = 0; i < result.fdRes.fdNum; i ++) {
            printf("fDParams.fdRes.fdBN2Idx[%d]: %d \n", i, result.fdRes.fdBN2Idx[i]);
            printf("fDParams.fdRes.fdMIdx[%d]: %d \n", i, result.fdRes.fdMIdx[i]);
            printf("fDParams.fdRes.fdS2SplitNum[%d]: %d \n", i, result.fdRes.fdS2SplitNum[i]);
            printf("fDParams.fdRes.fdWorkspaceIdx[%d]: %d \n", i, result.fdRes.fdWorkspaceIdx[i]);
            printf("fDParams.fdRes.mSize[%d]: %d \n", i, result.fdRes.mSize[i]);
        }

        for (uint32_t i = 0; i < result.fdRes.fdUsedVecNum; i ++) {
            printf("fDParams.fdRes.taskIdx[%d]: %d \n", i, result.fdRes.taskIdx[i]);
            printf("fDParams.fdRes.mStart[%d]: %d \n", i, result.fdRes.mStart[i]);
            printf("fDParams.fdRes.mLen[%d]: %d \n", i, result.fdRes.mLen[i]);
        }
    }
}