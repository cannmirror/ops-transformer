...
private:
        ...
        if (s1IdxEnd - s1IdxStart >= 1) {
            ...
            if (s1IdxEnd - s1IdxStart > 1) {
                CopyMultiMatrixNDToNZ(dstTensor.tensor[l1Offset], srcTensor.gmTensor[gmOffset],
                        s1IdxEnd - s1IdxStart - 1, offsetCalculator.GetStrideS1(), offsetCalculator.GetDimG() * blockElementCnt,
                                      ...
                                      GmCoord &gmCoord)
    {
        ...
        if (gIdxEnd - gIdxStart >= 1) {
            ...
            if (gIdxEnd - gIdxStart > 1) {
                CopyMultiMatrixNDToNZ(dstTensor.tensor[l1Offset], srcTensor.gmTensor[gmOffset],
                        gIdxEnd - gIdxStart - 1, offsetCalculator.GetStrideG(), s1Size * blockElementCnt,
...
template <typename T>
__aicore__ inline void CopySingleMXScaleDNToNZ(LocalTensor<T> l1Tensor, const GlobalTensor<T> gmTensor,
    uint32_t nValue, uint32_t dValue, uint32_t srcDValue, uint32_t dstNzC0Stride)
{
...
__aicore__ inline void CopyMultiMXScaleDNToNZ(LocalTensor<T> l1Tensor, const GlobalTensor<T> gmTensor,
    uint32_t srcNdMatrixNum, uint32_t srcDnMatrixStride, uint32_t dstNzMatrixStride, uint32_t nValue, uint32_t dValue, uint32_t srcDValue, uint32_t dstNzC0Stride)
{
    Dn2NzParams dn2nzPara;
...
public:
    __aicore__ inline void operator()(FaL1Tensor<Q_SCALE_T, L1_FORMAT> &dstTensor,
                                      FaGmTensor<Q_SCALE_T, GM_FORMAT> &srcTensor,
                                      GmCoord &gmCoord)
    {
...
private:
    __aicore__ inline void ProcessS1G(FaL1Tensor<Q_SCALE_T, L1_FORMAT> &dstTensor, FaGmTensor<Q_SCALE_T, GM_FORMAT> &srcTensor,
                                      GmCoord &gmCoord)
    {
        OffsetCalculator<GM_FORMAT> &offsetCalculator = srcTensor.offsetCalculator;
        uint32_t s1IdxStart = gmCoord.gS1Idx / offsetCalculator.GetDimG();
        ...
        uint64_t queryScaleGmbaseOffset =
            offsetCalculator.GetOffset(gmCoord.bIdx, gmCoord.n2Idx, 0, s1IdxStart, gmCoord.dIdx);
        CopySingleMXScaleDNToNZ(dstTensor.tensor, srcTensor.gmTensor[queryScaleGmbaseOffset], gmCoord.dDealSize, gmCoord.gS1DealSize,
                                    gmCoord.dDealSize, gmCoord.dDealSize);
    }
    ...
    __aicore__ inline void ProcessGS1(FaL1Tensor<Q_SCALE_T, L1_FORMAT> &dstTensor, FaGmTensor<Q_SCALE_T, GM_FORMAT> &srcTensor,
                                      GmCoord &gmCoord)
    {
        // N2*G*T(BS1)*D
        OffsetCalculator<GM_FORMAT> &offsetCalculator = srcTensor.offsetCalculator;
        ...
        if (gIdxEnd - gIdxStart >= 1) {
            ...
            if (gIdxEnd - gIdxStart > 1) {
                CopyMultiMXScaleDNToNZ(dstTensor.tensor[l1Offset], srcTensor.gmTensor[gmOffset],
                        gIdxEnd - gIdxStart - 1, offsetCalculator.GetStrideG(), 1,
                        1, s1Size, offsetCalculator.GetStrideS1(), offsetCalculator.GetDimD());
                gmOffset += (gIdxEnd - gIdxStart - 1) * offsetCalculator.GetStrideG();
            ...
            // 处理尾块
            if (s1IdxEnd > 0) {
                CopySingleMXScaleDNToNZ(dstTensor.tensor[l1Offset], srcTensor.gmTensor[gmOffset],
                                       gmCoord.dDealSize, s1IdxEnd, offsetCalculator.GetStrideS1(), gmCoord.dDealSize);
            }
        }
    }
};
...
// ----------------------------------------------CopyKeyScaleGmToL1--------------------------------
template <typename K_SCALE_T, GmFormat GM_FORMAT, L1Format L1_FORMAT = L1Format::NZ, ScaleTrans SCALE_TRANS = ScaleTrans::DN2NZ>
                                                ...
                                                GmKvCoord &gmCoord)
    {
        ...
        if constexpr (GM_FORMAT == GmFormat::PA_NZ_K_SCALE) {
            ...
            while (copyFinishRowCnt < gmCoord.s2DealSize) {
                ...
                // 拷贝数据
                CopySingleMXScaleDNToNZ(dstTensor.tensor[l1Offset], srcTensor.gmTensor[gmOffset], gmCoord.dDealSize, copyRowCnt,
                                        offsetCalculator.GetStrideBlockSize(), gmCoord.dDealSize);
...
template <typename T>
__aicore__ inline void CopySingleMXScaleNDToNZ(LocalTensor<T> l1Tensor, const GlobalTensor<T> gmTensor,
    uint32_t nValue, uint32_t dValue, uint32_t srcDValue, uint32_t dstNzC0Stride)
{
    Nd2NzParams nd2nzPara;
    nd2nzPara.ndNum = 1;
...
template <typename V_SCALE_T, GmFormat GM_FORMAT, L1Format L1_FORMAT = L1Format::NZ, ScaleTrans SCALE_TRANS = ScaleTrans::ND2NZ>
                                                ...
                                                GmKvCoord &gmCoord)
    {
        ...
        while (copyFinishRowCnt < gmCoord.s2DealSize) {
            ...
            if constexpr (GM_FORMAT == GmFormat::PA_NZ) {
                ...
                if (SCALE_TRANS == ScaleTrans::ND2NZ) {
                    CopySingleMXScaleNDToNZ(dstTensor.tensor[l1Offset], srcTensor.gmTensor[gmOffset], copyRowCnt, gmCoord.dDealSize,
                        offsetCalculator.GetStrideBlockSize(), dstTensor.rowCount);
                } else if (SCALE_TRANS == ScaleTrans::DN2NZ) {
                    CopySingleMXScaleDNToNZ(dstTensor.tensor[l1Offset], srcTensor.gmTensor[gmOffset], gmCoord.dDealSize, copyRowCnt,
                        offsetCalculator.GetStrideBlockSize(), gmCoord.dDealSize);
        }
