...
__simd_vf__ void ProcessVec1NoUpdateGeneralImpl128Mxfp8FullquantVFSubloop0(
    ...
    __ubuf__ T * maxUb, __ubuf__ T * maxUbStart, __ubuf__ T * srcUb, __ubuf__ T * expSumUb,
...
__simd_vf__ void ProcessVec1NoUpdateGeneralImpl128Mxfp8FullquantVFSubloop1(
    ...
    const uint32_t tailN, const float dScale, uint32_t pltOriTailN, uint32_t pltTailN, float divValue, 
    const uint16_t m, const uint32_t pseStride, const float slopes, const float posShift, const T scale, 
    const float dScaleQK, const T minValue,
    const float deSCaleKValue = 1.0f, const float sinkValue = 0.0f, const float pScale = 1.0f)
{
    RegTensor<float> vreg_min;
    RegTensor<float> vreg_sel;
    ...
    const float sinkValue = 0.0f, const float pScale = 1.0f)
{
    ...
    if (subLoop == 0){
        ProcessVec1NoUpdateGeneralImpl128Mxfp8FullquantVFSubloop0<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop,
        ...
        ProcessVec1NoUpdateGeneralImpl128Mxfp8FullquantVFSubloop1<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop,
            isMlaSgd, isMlaFullQuant, hasSink>(
            expUb, x_expUb, pseUb, maxUb, maxUbStart, srcUb, expMaxUb, inMaxUb, expSumUb, inExpSumUb, tmpExpSumUb, 
            tmpExpSumUb2, tmpMaxUb, tmpMaxUb2,  indexesUb, maskUb, maskUbUnroll, dropMaskUb, nPadding, 
            blockStride, repeatStride, oriTailN, tailN, dScale, pltOriTailN, pltTailN, divValue, m, 
