    ...
    const float pScale = 1.0f)
{
    ...
    // TODO：这里都需要检查,preg均需要关注subLoop == 0) {
    ...
    const float deSCaleKValue = 1.0f, const float sinkValue = 0.0f, const float pScale = 1.0f)
{
    ...
    // TODO：这里都需要检查,preg均需要关注subLoop == 1) {
    StoreUnAlignPost<T, MicroAPI::PostLiteral::POST_MODE_UPDATE>(
...
__aicore__ inline void ProcessVec1NoUpdateImpl128Mxfp8Fullquant(
    ...
    const LocalTensor<uint8_t>& sharedTmpBuffer, uint32_t subLoop,
    ...
    const float sinkValue = 0.0f, const float pScale = 1.0f)
{
    ...
    if (subLoop == 0){
        ProcessVec1NoUpdateImpl128Mxfp8FullquantVFSubloop0<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop,
            isMlaSgd, isMlaFullQuant, hasSink>(
            expUb, x_expUb, pseUb, maxUb, maxUbStart, srcUb, expSumUb, indexesUb, maskUb, maskUbUnroll,
            dropMaskUb, divValue, blockStride, repeatStride, dScale, m, pseStride, slopes, posShift,
            scale, dScaleQK, minValue, deSCaleKValue, sinkValue, pScale);
    } else {
        ProcessVec1NoUpdateImpl128Mxfp8FullquantVFSubloop1<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop,
            isMlaSgd, isMlaFullQuant, hasSink>(
            ...
            divValue, blockStride, repeatStride, dScale, m, pseStride, slopes, posShift, scale, dScaleQK,
