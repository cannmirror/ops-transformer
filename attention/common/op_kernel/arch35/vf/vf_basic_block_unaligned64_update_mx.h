...
__simd_vf__ void ProcessVec1UpdateImpl64Mxfp8FullquantVFSubloop1(
    ...
    __ubuf__ T * tmpMaxUb2, __ubuf__ uint8_t * indexesUb, __ubuf__ uint32_t * maskUb,
    __ubuf__ uint32_t * dropMaskUb, __ubuf__ float *preLoopMaxUb, __ubuf__ float *preLoopSumUb,
    __ubuf__ float *firstLoopSumUb, const uint32_t nPadding, const uint32_t blockStride,
    const uint32_t repeatStride, const float dScale, uint32_t pltOriginalN, float divValue, 
    ...
    const float deSCaleKValue = 1.0f, const float sinkValue = 0.0f, const float pScale = 1.0f)
{
    ...
    if (subLoop == 0) {
        ProcessVec1UpdateImpl64Mxfp8FullquantVFSubloop0<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop,
        ...
        ProcessVec1UpdateImpl64Mxfp8FullquantVFSubloop1<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop,
            isMlaSgd, isMlaFullQuant, hasSink>(
            expUb, pseUb, maxUb, maxUbStart, srcUb, expMaxUb, inMaxUb, expSumUb, inExpSumUb, tmpExpSumUb, tmpExpSumUb2,
            tmpMaxUb, tmpMaxUb2, indexesUb, maskUb, dropMaskUb, preLoopMaxUb, preLoopSumUb, firstLoopSumUb, nPadding,
            blockStride, repeatStride, dScale, pltOriginalN, divValue, pltSrcN, pltSrcN16, m, pseStride, slopes,
