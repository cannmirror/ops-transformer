...
__simd_vf__ void ProcessVec1UpdateImpl128Mxfp8FullquantVFSubloop0(
    __ubuf__ T2 * expUb, __ubuf__ T2 * x_expUb, __ubuf__ pseShiftType * pseUb, __ubuf__ T * maxUb,
    __ubuf__ T * maxUbStart, __ubuf__ T * srcUb, __ubuf__ T * expMaxUb, __ubuf__ T * inMaxUb,
    __ubuf__ T * expSumUb, __ubuf__ T * inExpSumUb, __ubuf__ T * tmpExpSumUb, __ubuf__ T * tmpExpSumUb2,
    __ubuf__ T * tmpMaxUb, __ubuf__ T * tmpMaxUb2, __ubuf__ uint8_t * indexesUb, __ubuf__ uint32_t * maskUb,
    __ubuf__ uint32_t * maskUbUnroll, __ubuf__ uint32_t * dropMaskUb, __ubuf__ float *preLoopMaxUb,
    __ubuf__ float *preLoopSumUb, __ubuf__ float *firstLoopSumUb, 
    float divValue, const uint32_t blockStride,const uint32_t repeatStride, const float dScale, const uint16_t m, 
    ...
    const T minValue,  const float deSCaleKValue = 1.0f, const float sinkValue = 0.0f, const float pScale = 1.0f)
{
    ...
    StoreAlign<T, MicroAPI::StoreDist::DIST_NORM_B32>(
        (__ubuf__ T *&)preLoopMaxUb, vreg_in_max, preg_all); // loop 0的时候，把之前的全局最大值写入ub
    ...
    StoreAlign<T, MicroAPI::StoreDist::DIST_NORM_B32>(
        (__ubuf__ T *&)preLoopSumUb, vreg_in_exp_sum, preg_all);//之前的sum值写到上一轮loop中
    Mul(vreg_exp_max, vreg_exp_max, vreg_in_exp_sum, preg_all);//用之前的（全局最大值-本轮max）*之前的sum
    //用之前的全局最大值*之前的sum +本轮sum
    Add(vreg_exp_max, vreg_exp_max, vreg_exp_sum_brc, preg_all); // loop 0更新后的sum值，暂时用不到，需要的是loop 0自己的sum值
    //存subloop=0的sum
    StoreAlign<T, MicroAPI::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)expSumUb, vreg_exp_max, preg_all);
    //subloop=0的纯sum值
    StoreAlign<T, MicroAPI::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)firstLoopSumUb, vreg_exp_sum_brc, preg_all); // 传给loop 1用于更新rowSum
}
    ...
    const T minValue,  const float deSCaleKValue = 1.0f, const float sinkValue = 0.0f, const float pScale = 1.0f)
{
    ...
    LoadAlign(vreg_pre_loop_max, preLoopMaxUb);//应该是 [4100d376]	Address 00000004 = [415071a3]其实Address 00000000 = [0000d376]	Address 00000004 = [000071a3]
...
__aicore__ inline void ProcessVec1UpdateImpl128Mxfp8Fullquant(
    ...
    const LocalTensor<float>& preLoopMaxTensor, const LocalTensor<float>& preLoopSumTensor, const LocalTensor<float>& firstLoopSumTensor, uint32_t subLoop,
    const uint16_t m,
    ...
    const float deSCaleKValue = 1.0f, const float sinkValue = 0.0f, const float pScale = 1.0f)
{
    ...
    if (subLoop == 0) {
        ProcessVec1UpdateImpl128Mxfp8FullquantVFSubloop0<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop,
            isMlaSgd, isMlaFullQuant, hasSink>(
            expUb, x_expUb, pseUb, maxUb, maxUbStart, srcUb, expMaxUb, inMaxUb, expSumUb, inExpSumUb, 
            tmpExpSumUb, tmpExpSumUb2, tmpMaxUb, tmpMaxUb2, indexesUb, maskUb, maskUbUnroll, dropMaskUb,
            preLoopMaxUb, preLoopSumUb, firstLoopSumUb, divValue, blockStride, repeatStride, dScale, m,
            pseStride, slopes, posShift, scale, dScaleQK, minValue, deSCaleKValue, sinkValue, pScale);
    } else {
        ProcessVec1UpdateImpl128Mxfp8FullquantVFSubloop1<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop,
            isMlaSgd, isMlaFullQuant, hasSink>(
            expUb, x_expUb, pseUb, maxUb, maxUbStart, srcUb, expMaxUb, inMaxUb, expSumUb, inExpSumUb,
            tmpExpSumUb, tmpExpSumUb2, tmpMaxUb, tmpMaxUb2, indexesUb, maskUb, maskUbUnroll, dropMaskUb,
            preLoopMaxUb, preLoopSumUb, firstLoopSumUb, divValue, blockStride, repeatStride, dScale, m,
