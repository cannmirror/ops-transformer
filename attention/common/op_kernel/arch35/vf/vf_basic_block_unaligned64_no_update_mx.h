...
__simd_vf__ void ProcessVec1NoUpdateImpl64Mxfp8FullquantVFSubloop0(
    __ubuf__ T2 * expUb, __ubuf__ pseShiftType * pseUb, __ubuf__ T * maxUb, __ubuf__ T * maxUbStart,
    __ubuf__ T * srcUb, __ubuf__ T * expSumUb, __ubuf__ uint8_t * indexesUb, __ubuf__ uint32_t * maskUb,
    __ubuf__ uint32_t * dropMaskUb, const uint32_t blockStride, const uint32_t repeatStride, 
...
__simd_vf__ void ProcessVec1NoUpdateImpl64Mxfp8FullquantVFSubloop1(
    __ubuf__ T2 * expUb, __ubuf__ pseShiftType * pseUb, __ubuf__ T * maxUb, __ubuf__ T * maxUbStart,
    __ubuf__ T * srcUb, __ubuf__ T * expMaxUb, __ubuf__ T * inMaxUb, __ubuf__ T * expSumUb, 
    __ubuf__ T * inExpSumUb, __ubuf__ T * tmpExpSumUb, __ubuf__ T * tmpExpSumUb2, __ubuf__ T * tmpMaxUb,
    __ubuf__ T * tmpMaxUb2, __ubuf__ uint8_t * indexesUb,__ubuf__ uint32_t * maskUb, __ubuf__ uint32_t * dropMaskUb, 
    ...
    const T minValue,const float deSCaleKValue = 1.0f, const float sinkValue = 0.0f, const float pScale = 1.0f)
{
    ...
    // TODO：这里都需要检查,preg均需要关注 subLoop == 1) {
    StoreUnAlignPost<T, MicroAPI::PostLiteral::POST_MODE_UPDATE>(
    ...
    const float sinkValue = 0.0f, const float pScale = 1.0f)
{
    ...
    if (subLoop == 0) {
        ProcessVec1NoUpdateImpl64Mxfp8FullquantVFSubloop0<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop,
            isMlaSgd, isMlaFullQuant, hasSink>(
            expUb, pseUb, maxUb, maxUbStart, srcUb, expSumUb, indexesUb, maskUb, dropMaskUb, blockStride,
            repeatStride, nPadding, pltOriginalN, divValue, pltSrcN, pltSrcN16, dScale, m, pseStride,
            slopes, posShift, scale, dScaleQK, minValue, deSCaleKValue, sinkValue, pScale);
    } else {
        ProcessVec1NoUpdateImpl64Mxfp8FullquantVFSubloop1<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop,
            isMlaSgd, isMlaFullQuant, hasSink>(
            expUb, pseUb, maxUb, maxUbStart, srcUb, expMaxUb, inMaxUb, expSumUb, inExpSumUb, tmpExpSumUb,
            tmpExpSumUb2, tmpMaxUb, tmpMaxUb2, indexesUb, maskUb, dropMaskUb, blockStride, repeatStride,
            nPadding, pltOriginalN, divValue, pltSrcN, pltSrcN16, dScale, m, pseStride, slopes, posShift,
