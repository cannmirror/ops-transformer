...
uint8_t Mc2GetCommAlgo(int64_t rankDim, uint64_t mValue, const char *group, const gert::TilingContext *context)
{
    ...
    if (rankDim == 2) { // 如果是2p
        if ((debugCommAlg != 0) && (debugCommAlg != COMM_ALG_FULL_MESH)) {
            OP_LOGE(context->GetNodeName(), " CommAlgo %u is not supported when rank dim is 2.", debugCommAlg);
            return COMM_ALG_DEFAULT;
        }
        return COMM_ALG_FULL_MESH;
    } else if (rankDim <= 0) {
        OP_LOGE(context->GetNodeName(), "Invalid rank dimension %lld. Rank dimension must be positive.", rankDim);
        return COMM_ALG_DEFAULT;
    }
    ...
    if (commSets == COMM_MESH) {
        if ((debugCommAlg != 0) && (debugCommAlg != COMM_ALG_FULL_MESH)) { // 环境变量设置非fullmesh
            OP_LOGE(context->GetNodeName(), " CommAlg %u is not supported.", debugCommAlg);
            return COMM_ALG_DEFAULT;
        }
        return COMM_ALG_FULL_MESH;
    }
...
ge::graphStatus GetMatmulV3PriorityPolicy(const NpuArch npuArch, std::vector<int32_t> &priorities, const char *opName)
{
    ...
    if (priorities.empty()) {
        OP_LOGE(opName, "version %u can't find suitable matmul priorities", static_cast<uint32_t>(npuArch));
...
ge::graphStatus Mc2TilingUtils::CommonParamCheck(const gert::TilingContext *context)
{
    ...
    OP_TILING_CHECK(aShape == nullptr || bShape == nullptr, OP_LOGE(context->GetNodeName(), "the shape is invalid"),
                    return ge::GRAPH_FAILED);

    uint64_t aShapeDimNum = aShape->GetStorageShape().GetDimNum();
    uint64_t bShapeDimNum = bShape->GetStorageShape().GetDimNum();
    OP_TILING_CHECK(aShapeDimNum != 2 || bShapeDimNum != 2, OP_LOGE(context->GetNodeName(), "the dimNum is not two"),
                    return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(aTensor == nullptr || bTensor == nullptr || output == nullptr,
                    OP_LOGE(context->GetNodeName(), "the tensor is invalid"), return ge::GRAPH_FAILED);
    ...
    OP_TILING_CHECK(aShapeFormat != outputFormat,
                    OP_LOGE(context->GetNodeName(), "a shape Format, output Format are not same"),
                    return ge::GRAPH_FAILED);
    OP_TILING_CHECK(
        (mc2tiling::SUPPORTED_FORMAT.count(aShapeFormat) == 0 || mc2tiling::SUPPORTED_FORMAT.count(bShapeFormat) == 0),
        OP_LOGE(context->GetNodeName(), "a shape Format, b shape Format only support ND, the format is %s",
