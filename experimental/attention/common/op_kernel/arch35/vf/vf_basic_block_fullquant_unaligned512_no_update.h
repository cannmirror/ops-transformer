...
__simd_vf__ void ProcessVec1NoUpdateGeneralImpl512NZVF(
    ...
    const uint16_t m, const uint32_t n, const uint32_t tailM, uint32_t tailMSize1, uint32_t tailMSize2, uint32_t tailMAddSize,
    const uint32_t secondN, const uint32_t secondFracMulti, const uint32_t isAlignCast, const T minValue, const float pScale)
{
    ...
    MaskReg preg_tail_m = UpdateMask<uint16_t>(tailMSize1);
    MaskReg preg_tail_m_b8 = UpdateMask<uint8_t>(tailMSize2);
    MaskReg preg_tail_m_add_fp32 = UpdateMask<uint32_t>(tailMAddSize);
    ...
    if (tailM != 0) {  // m非16对齐
        __ubuf__ T * srcUbTailM1 = srcUb1 + (m / 16) * 16 * 16;
        __ubuf__ T * srcUbTailM2 = srcUb2 + (m / 16) * 16 * 16;
        LoadAlign(vreg_input_x_1, srcUbTailM1);
        LoadAlign(vreg_input_x_2, srcUbTailM2);
        if (tailM <= 8) {
            Select(vreg_input_x_1, vreg_input_x_1, vreg_min, preg_tail_m);
            Select(vreg_input_x_2, vreg_input_x_2, vreg_min, preg_tail_m);
            Max(vreg_max_tmp, vreg_input_x_1, vreg_input_x_2, preg_tail_m);
            for (uint16_t j = 1; j < n / 16; ++j) {
                LoadAlign(vreg_input_x_1, srcUbTailM1 + j * m * 16);
                Select(vreg_input_x_1, vreg_input_x_1, vreg_min, preg_tail_m);
                Max(vreg_max_tmp, vreg_max_tmp, vreg_input_x_1, preg_tail_m);
                if (j < secondFracMulti) {          // 第二个256块有数据的分形
                    LoadAlign(vreg_input_x_2, srcUbTailM2 + j * m * 16);
                    Select(vreg_input_x_2, vreg_input_x_2, vreg_min, preg_tail_m);
                    Max(vreg_max_tmp, vreg_max_tmp, vreg_input_x_2, preg_tail_m);
                }
            }
            ReduceDataBlock<AscendC::MicroAPI::ReduceType::MAX>(vreg_max_tmp, vreg_max_tmp, preg_all);
            Sub(vreg_max_tmp, vreg_max_tmp, vreg_ln_p_scale, preg_all);
            StoreUnAlign<half, MicroAPI::PostLiteral::POST_MODE_UPDATE>(
                ((__ubuf__ half *&)maxUb), vreg_max_tmp, ureg_max, tailM);
        } else {
            Max(vreg_max_tmp, vreg_input_x_1, vreg_input_x_2, preg_all);
            LoadAlign(vreg_input_x_unroll_1, srcUbTailM1 + 8 * 16);
            LoadAlign(vreg_input_x_unroll_2, srcUbTailM2 + 8 * 16);
            Select(vreg_input_x_unroll_1, vreg_input_x_unroll_1, vreg_min, preg_tail_m);
            Select(vreg_input_x_unroll_2, vreg_input_x_unroll_2, vreg_min, preg_tail_m);
            Max(vreg_max_tmp_unroll, vreg_input_x_unroll_1, vreg_input_x_unroll_2, preg_tail_m);
            for (uint16_t j = 1; j < n / 16; ++j) {
                LoadAlign(vreg_input_x_1, srcUbTailM1 + j * m * 16);
                LoadAlign(vreg_input_x_unroll_1, srcUbTailM1 + j *m * 16 + 8 * 16);
                Max(vreg_max_tmp, vreg_max_tmp, vreg_input_x_1, preg_all);
                Max(vreg_max_tmp_unroll, vreg_max_tmp_unroll, vreg_input_x_unroll_1, preg_tail_m);
                if (j < secondFracMulti) {
                    LoadAlign(vreg_input_x_2, srcUbTailM2 + j * m * 16);
                    LoadAlign(vreg_input_x_unroll_2, srcUbTailM2 + j * m * 16 + 8 * 16);
                    Max(vreg_max_tmp, vreg_max_tmp, vreg_input_x_2, preg_all);
                    Max(vreg_max_tmp_unroll, vreg_max_tmp_unroll, vreg_input_x_unroll_2, preg_tail_m);
                }
            }
            ReduceDataBlock<AscendC::MicroAPI::ReduceType::MAX>(vreg_max_tmp, vreg_max_tmp, preg_all);
            ReduceDataBlock<AscendC::MicroAPI::ReduceType::MAX>(vreg_max_tmp_unroll, vreg_max_tmp_unroll, preg_all);
            Sub(vreg_max_tmp, vreg_max_tmp, vreg_ln_p_scale, preg_all);
            Sub(vreg_max_tmp_unroll, vreg_max_tmp_unroll, vreg_ln_p_scale, preg_tail_m);
            StoreUnAlign<half, MicroAPI::PostLiteral::POST_MODE_UPDATE>(
                ((__ubuf__ half *&)maxUb), vreg_max_tmp, ureg_max, 8);  // 写回16行max
            StoreUnAlign<half, MicroAPI::PostLiteral::POST_MODE_UPDATE>(
                ((__ubuf__ half *&)maxUb), vreg_max_tmp_unroll, ureg_max, tailM - 8);
        }
    }
    StoreUnAlignPost<half, MicroAPI::PostLiteral::POST_MODE_UPDATE>(
            ((__ubuf__ half *&)maxUb), ureg_max, 0);
    ...
    if (tailM != 0) {
        ...
        Duplicate(vreg_exp_sum_1, 0, preg_all);  // sum清零
        Duplicate(vreg_exp_sum_2, 0, preg_all);
        LoadAlign<half, MicroAPI::LoadDist::DIST_E2B_B16>(vreg_max, maxUb2 + (m / 16) * 16);  // 读入max并广播
        if (tailM <= 8) {
            Select(vreg_max, vreg_max, vreg_min, preg_tail_m);
            // 第一个256
            for (uint16_t j = 0; j < n / 32; ++j) {
                LoadAlign<half, MicroAPI::LoadDist::DIST_NORM>(
                    vreg_input_x_1, srcUbTailM1+ j * m * 32); // 第一个分形的前8行
                LoadAlign<half, MicroAPI::LoadDist::DIST_NORM>(
                    vreg_input_x_unroll_1, srcUbTailM1 + j * m * 32 + m * 16);  // 第二个分形的前8行
                Select(vreg_input_x_1, vreg_input_x_1, vreg_min, preg_tail_m);
                Select(vreg_input_x_unroll_1, vreg_input_x_unroll_1, vreg_min, preg_tail_m);

                ExpSub<float, half, RegLayout::ZERO>(vreg_exp_0_1, vreg_input_x_1, vreg_max, preg_tail_m);
                ExpSub<float, half, RegLayout::ONE>(vreg_exp_2_1, vreg_input_x_1, vreg_max, preg_tail_m);
                ExpSub<float, half, RegLayout::ZERO>(vreg_exp_1_1, vreg_input_x_unroll_1, vreg_max, preg_tail_m);
                ExpSub<float, half, RegLayout::ONE>(vreg_exp_3_1, vreg_input_x_unroll_1, vreg_max, preg_tail_m);
                Add(vreg_exp_sum_1, vreg_exp_sum_1, vreg_exp_0_1, preg_tail_m_add_fp32);
                Add(vreg_exp_sum_1, vreg_exp_sum_1, vreg_exp_2_1, preg_tail_m_add_fp32);
                Add(vreg_exp_sum_1, vreg_exp_sum_1, vreg_exp_1_1, preg_tail_m_add_fp32);
                Add(vreg_exp_sum_1, vreg_exp_sum_1, vreg_exp_3_1, preg_tail_m_add_fp32);
                ...
                LoadAlign(vreg_exp_merge_f8_indexes, indexesUb);
                Gather(vreg_exp_merge_f8_1, vreg_exp_merge_f8_1, vreg_exp_merge_f8_indexes);
                StoreAlign(expUb1 + (m / 16) * 16 * 32 + j * m * 32, vreg_exp_merge_f8_1, preg_tail_m_b8);
            }
            // 第二个256
            for (uint16_t j = 0; j < n / 32; ++j) {
                LoadAlign<half, MicroAPI::LoadDist::DIST_NORM>(
                    vreg_input_x_1, srcUbTailM2 + j * m * 32);
                LoadAlign<half, MicroAPI::LoadDist::DIST_NORM>(
                    vreg_input_x_unroll_1, srcUbTailM2 + j * m * 32 + m * 16);
                Select(vreg_input_x_1, vreg_input_x_1, vreg_min, preg_tail_m);
                Select(vreg_input_x_unroll_1, vreg_input_x_unroll_1, vreg_min, preg_tail_m);
                ...
                ExpSub<float, half, RegLayout::ZERO>(vreg_exp_1_1, vreg_input_x_unroll_1, vreg_max, preg_tail_m);
                ExpSub<float, half, RegLayout::ONE>(vreg_exp_3_1, vreg_input_x_unroll_1, vreg_max, preg_tail_m);
                Add(vreg_exp_sum_1, vreg_exp_sum_1, vreg_exp_0_1, preg_tail_m_add_fp32);  // tailMSize的一半元素数，但为fp32
                Add(vreg_exp_sum_1, vreg_exp_sum_1, vreg_exp_2_1, preg_tail_m_add_fp32);
                Add(vreg_exp_sum_1, vreg_exp_sum_1, vreg_exp_1_1, preg_tail_m_add_fp32);
                Add(vreg_exp_sum_1, vreg_exp_sum_1, vreg_exp_3_1, preg_tail_m_add_fp32);
                ...
                LoadAlign(vreg_exp_merge_f8_indexes, indexesUb);
                Gather(vreg_exp_merge_f8_1, vreg_exp_merge_f8_1, vreg_exp_merge_f8_indexes);
                StoreAlign(expUb2 + (m / 16) * 16 * 32 + j * m * 32, vreg_exp_merge_f8_1, preg_tail_m_b8);
            }
            ReduceDataBlock<AscendC::MicroAPI::ReduceType::SUM>(vreg_exp_sum_1, vreg_exp_sum_1, preg_all);
            StoreUnAlign<float, MicroAPI::PostLiteral::POST_MODE_UPDATE>(
                ((__ubuf__ float *&)expSumUb), vreg_exp_sum_1, ureg_exp_sum, tailM);
        } else {
            LoadAlign<half, MicroAPI::LoadDist::DIST_E2B_B16>(vreg_max_2, maxUb2 + (m / 16) * 16 + 8);
            Select(vreg_max_2, vreg_max_2, vreg_min, preg_tail_m);
            // 第一个256
            for (uint16_t j = 0; j < n / 32; ++j) {
                LoadAlign<half, MicroAPI::LoadDist::DIST_NORM>(
                    vreg_input_x_1, srcUbTailM1+ j * m * 32); // 第一个分形的前8行
                ...
                LoadAlign<half, MicroAPI::LoadDist::DIST_NORM>(
                    vreg_input_x_2, srcUbTailM1 + j * m * 32 + 8 * 16);  // 后几行
                LoadAlign<half, MicroAPI::LoadDist::DIST_NORM>(
                    vreg_input_x_unroll_2, srcUbTailM1 + j * m * 32 + m * 16 + 8 * 16);
                Select(vreg_input_x_2, vreg_input_x_2, vreg_min, preg_tail_m);
                Select(vreg_input_x_unroll_2, vreg_input_x_unroll_2, vreg_min, preg_tail_m);
                ...
                ExpSub<float, half, RegLayout::ZERO>(vreg_exp_0_2, vreg_input_x_2, vreg_max_2, preg_tail_m);
                ExpSub<float, half, RegLayout::ONE>(vreg_exp_2_2, vreg_input_x_2, vreg_max_2, preg_tail_m);
                ExpSub<float, half, RegLayout::ZERO>(vreg_exp_1_2, vreg_input_x_unroll_2, vreg_max_2, preg_tail_m);
                ExpSub<float, half, RegLayout::ONE>(vreg_exp_3_2, vreg_input_x_unroll_2, vreg_max_2, preg_tail_m);
                ...
                Add(vreg_exp_sum_2, vreg_exp_sum_2, vreg_exp_0_2, preg_tail_m_add_fp32);
                Add(vreg_exp_sum_2, vreg_exp_sum_2, vreg_exp_2_2, preg_tail_m_add_fp32);
                Add(vreg_exp_sum_2, vreg_exp_sum_2, vreg_exp_1_2, preg_tail_m_add_fp32);
                Add(vreg_exp_sum_2, vreg_exp_sum_2, vreg_exp_3_2, preg_tail_m_add_fp32);
                ...
                LoadAlign(vreg_exp_merge_f8_indexes, indexesUb);
                Gather(vreg_exp_merge_f8_1, vreg_exp_merge_f8_1, vreg_exp_merge_f8_indexes);
                StoreAlign(expUb1 + (m / 16) * 16 * 32 + j * m * 32, vreg_exp_merge_f8_1, preg_all);
                ...
                Or((RegTensor<uint8_t>&)vreg_exp_merge_f8_2, (RegTensor<uint8_t>&)vreg_exp_merge_tmp_f8_2_1, (RegTensor<uint8_t>&)vreg_exp_merge_tmp_f8_2_2, preg_all_b8);
                Gather(vreg_exp_merge_f8_2, vreg_exp_merge_f8_2, vreg_exp_merge_f8_indexes);
                StoreAlign(expUb1 + (m / 16) * 16 * 32 + j * m * 32 + 8 * 32, vreg_exp_merge_f8_2, preg_tail_m_b8);
            }
            // 第二个256
            for (uint16_t j = 0; j < n / 32; ++j) {
                LoadAlign<half, MicroAPI::LoadDist::DIST_NORM>(
                    vreg_input_x_1, srcUbTailM2 + j * m * 32);
                LoadAlign<half, MicroAPI::LoadDist::DIST_NORM>(
                    vreg_input_x_2, srcUbTailM2 + j * m * 32 + 8 * 16);
                LoadAlign<half, MicroAPI::LoadDist::DIST_NORM>(
                    vreg_input_x_unroll_1, srcUbTailM2 + j * m * 32 + m * 16);
                LoadAlign<half, MicroAPI::LoadDist::DIST_NORM>(
                        vreg_input_x_unroll_2, srcUbTailM2 + j * m * 32 + m * 16 + 8 * 16);
                Select(vreg_input_x_2, vreg_input_x_2, vreg_min, preg_tail_m);
                Select(vreg_input_x_unroll_2, vreg_input_x_unroll_2, vreg_min, preg_tail_m);
                ...
                LoadAlign(vreg_exp_merge_f8_indexes, indexesUb);
                Gather(vreg_exp_merge_f8_1, vreg_exp_merge_f8_1, vreg_exp_merge_f8_indexes);
                StoreAlign(expUb2 + (m / 16) * 16 * 32 + j * m * 32, vreg_exp_merge_f8_1, preg_all_b8);
                ...
                Or((RegTensor<uint8_t>&)vreg_exp_merge_f8_2, (RegTensor<uint8_t>&)vreg_exp_merge_tmp_f8_2_1, (RegTensor<uint8_t>&)vreg_exp_merge_tmp_f8_2_2, preg_all_b8);
                Gather(vreg_exp_merge_f8_2, vreg_exp_merge_f8_2, vreg_exp_merge_f8_indexes);
                StoreAlign(expUb2 + (m / 16) * 16 * 32 + j * m * 32 + 8 * 32, vreg_exp_merge_f8_2, preg_tail_m_b8);
            }
            ReduceDataBlock<AscendC::MicroAPI::ReduceType::SUM>(vreg_exp_sum_1, vreg_exp_sum_1, preg_all);
            ReduceDataBlock<AscendC::MicroAPI::ReduceType::SUM>(vreg_exp_sum_2, vreg_exp_sum_2, preg_all);
            StoreUnAlign<float, MicroAPI::PostLiteral::POST_MODE_UPDATE>(
                ((__ubuf__ float *&)expSumUb), vreg_exp_sum_1, ureg_exp_sum, 8);
            StoreUnAlign<float, MicroAPI::PostLiteral::POST_MODE_UPDATE>(
                ((__ubuf__ float *&)expSumUb), vreg_exp_sum_2, ureg_exp_sum, tailM - 8);
        }
    }
...
{
    uint32_t tailM = m % 16;     // 尾块s1方向上的非对齐分形的大小
    uint32_t tailMSize = tailM * 16;     // 寄存器内的有效数据
    uint32_t tailMAddSize = tailM * 8;     // 寄存器内的有效数据
    if (tailM > 8) {
        tailMSize = (tailM - 8) * 16;
        tailMAddSize = (tailM - 8) * 8;
    }
    uint32_t tailMSize2 = 2 * tailMSize;
    
    uint32_t n = 256;
    uint32_t secondN = originN - n;                  // 第二个256部分可能 <= 256
    uint32_t secondFracMulti = (secondN + 15) / 16;  // 第二个256部分的分形个数
    uint32_t isAlignCast = secondFracMulti % 2;       // 是否32对齐进行cast操作
    ...
    ProcessVec1NoUpdateGeneralImpl512NZVF<T, T2, OUTPUT_T, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop, isMlaSgd>(
        ...
        m, n, tailM, tailMSize, tailMSize2, tailMAddSize, secondN, secondFracMulti, isAlignCast, minValue, pScale);
}
} // namespace
