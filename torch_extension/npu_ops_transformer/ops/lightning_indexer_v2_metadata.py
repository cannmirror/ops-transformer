...
class LightningIndexerV2MetadataOpBuilder(OpBuilder):
    def __init__(self):
        super(LightningIndexerV2MetadataOpBuilder, self).__init__("npu_lightning_indexer_v2_metadata")

    def sources(self):
        """Path to C++ source code."""
        return ['ops/csrc/lightning_indexer_v2_metadata.cpp']

    def schema(self) -> str:
        """PyTorch operator signature."""
        return "npu_lightning_indexer_v2_metadata(int num_heads_q, int num_heads_k, int head_dim, int topk, *, "       \
            "Tensor? cu_seqlens_q=None, Tensor? cu_seqlens_k=None, Tensor? seqused_q=None, Tensor? seqused_k=None, "   \
            "Tensor? cmp_residual_k=None, int batch_size=0, int max_seqlen_q=0, int max_seqlen_k=0, "                  \
            "str layout_q='BSND', str layout_k='BSND', int mask_mode=0, int cmp_ratio=1) -> Tensor"
    ...
    def register_meta(self):
        ...
        def npu_lightning_indexer_v2_metadata_meta(
            num_heads_q: int, num_heads_k: int, head_dim: int, topk: int,
...
lightning_indexer_v2_metadata_op_builder = LightningIndexerV2MetadataOpBuilder()
op_module = lightning_indexer_v2_metadata_op_builder.load()


@impl(AS_LIBRARY, lightning_indexer_v2_metadata_op_builder.name, "PrivateUse1")
def npu_lightning_indexer_v2_metadata(
    num_heads_q: int, num_heads_k: int, head_dim: int, topk: int,
    ...
    return op_module.npu_lightning_indexer_v2_metadata(
...
@torch.library.register_kernel("npu_ops_transformer::npu_lightning_indexer_v2_metadata", None)
def npu_lightning_indexer_v2_metadata_fallback(
    num_heads_q: int, num_heads_k: int, head_dim: int, topk: int,
    cu_seqlens_q: Optional[Tensor] = None, cu_seqlens_k: Optional[Tensor] = None, seqused_q: Optional[Tensor] = None,
    seqused_k: Optional[Tensor] = None, cmp_residual_k: Optional[Tensor] = None,
    batch_size: Optional[int] = None, max_seqlen_q: Optional[int] = None, max_seqlen_k: Optional[int] = None,
    ...
    # 调用 NPU 实现
    return npu_lightning_indexer_v2_metadata(
        num_heads_q, num_heads_k, head_dim, topk,
        cu_seqlens_q, cu_seqlens_k, seqused_q, seqused_k, cmp_residual_k,
        batch_size, max_seqlen_q, max_seqlen_k, layout_q, layout_k, mask_mode, cmp_ratio)
        
torch.compiler.allow_in_graph(npu_lightning_indexer_v2_metadata)